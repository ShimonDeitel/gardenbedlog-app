import XCTest
@testable import GardenBedLog

@MainActor
final class GardenBedLogTests: XCTestCase {
    var store: Store!
    ...
    func testAddIncreasesCount() {
        ...
        store.add(GardenBedLogItem(name: "Test", detail: "Detail", extra: "", date: Date()))
        XCTAssertEqual(store.items.count, before + 1)
    }

    func testDeleteRemovesItem() {
        let item = GardenBedLogItem(name: "ToDelete", detail: "Detail", extra: "", date: Date())
        store.add(item)
        store.delete(item)
    ...
    func testFreeLimitBlocksAdditionalItems() {
        for i in 0..<Store.freeLimit {
            store.add(GardenBedLogItem(name: "Item \(i)", detail: "d", extra: "", date: Date()))
        }
    ...
    func testUpdateModifiesExistingItem() {
        let item = GardenBedLogItem(name: "Original", detail: "Detail", extra: "", date: Date())
        store.add(item)
        var updated = item
